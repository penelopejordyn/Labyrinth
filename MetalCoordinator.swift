...
// MARK: - Coordinator
    ...
    // MARK: - Undo/Redo System
	    ...
	    enum UndoAction {
	        ...
	        case refineStrokes(oldStrokes: [Stroke], newStrokes: [Stroke], target: DrawingTarget)
	        case replaceStrokes(oldStrokes: [Stroke], newStrokes: [Stroke], insertionIndex: Int, target: DrawingTarget)
    ...
    // MARK: - Handwriting Refinement (MVP)
    var handwritingRefinementEnabled: Bool = false {
        didSet {
            if !handwritingRefinementEnabled {
                resetHandwritingRefinementState()
            }
        }
	    }
	    private let handwritingRefiner = HandwritingRefiner()
	    private let handwritingBeautifier = HandwritingBeautifier()
	    private(set) var lastHandwritingRefinementOCRText: String?
	    private var handwritingRefinementTimer: Timer?
	    private var handwritingRefinementGeneration: Int = 0
	    private var handwritingRefinementPendingTarget: DrawingTarget?
	    private var handwritingRefinementPendingStrokeIDs: [UUID] = []
    private let handwritingRefinementDelaySeconds: TimeInterval = 0.5
    ...
    let brushSettings = BrushSettings()
	    ...
	    func undo() {
	        ...
	        case .refineStrokes(let oldStrokes, _, let target):
	            switch target {
	            case .canvas(let frame):
	                for old in oldStrokes {
	                    guard let idx = frame.strokes.firstIndex(where: { $0.id == old.id }) else { continue }
	                    frame.strokes[idx] = old
	                }
	            case .card(let card, _):
	                for old in oldStrokes {
	                    guard let idx = card.strokes.firstIndex(where: { $0.id == old.id }) else { continue }
	                    card.strokes[idx] = old
	                }
	            }
	            redoStack.append(action)

	        case .replaceStrokes(let oldStrokes, let newStrokes, let insertionIndex, let target):
	            let newIDs = Set(newStrokes.map { $0.id })
	            switch target {
	            case .canvas(let frame):
	                frame.strokes.removeAll { newIDs.contains($0.id) }
	                frame.strokes.insert(contentsOf: oldStrokes, at: min(insertionIndex, frame.strokes.count))
	            case .card(let card, _):
	                card.strokes.removeAll { newIDs.contains($0.id) }
	                card.strokes.insert(contentsOf: oldStrokes, at: min(insertionIndex, card.strokes.count))
	            }
	            redoStack.append(action)
	    ...
	    func redo() {
	        ...
	        case .refineStrokes(_, let newStrokes, let target):
	            switch target {
	            case .canvas(let frame):
	                for new in newStrokes {
	                    guard let idx = frame.strokes.firstIndex(where: { $0.id == new.id }) else { continue }
	                    frame.strokes[idx] = new
	                }
	            case .card(let card, _):
	                for new in newStrokes {
	                    guard let idx = card.strokes.firstIndex(where: { $0.id == new.id }) else { continue }
	                    card.strokes[idx] = new
	                }
	            }
	            undoStack.append(action)

	        case .replaceStrokes(let oldStrokes, let newStrokes, let insertionIndex, let target):
	            let oldIDs = Set(oldStrokes.map { $0.id })
	            switch target {
	            case .canvas(let frame):
	                frame.strokes.removeAll { oldIDs.contains($0.id) }
	                frame.strokes.insert(contentsOf: newStrokes, at: min(insertionIndex, frame.strokes.count))
	            case .card(let card, _):
	                card.strokes.removeAll { oldIDs.contains($0.id) }
	                card.strokes.insert(contentsOf: newStrokes, at: min(insertionIndex, card.strokes.count))
	            }
	            undoStack.append(action)
    ...
    func handleTouchBegan(at point: CGPoint, touchType: UITouch.TouchType) {
        ...
        cancelPendingHandwritingRefinement()
    }
    ...
    func handleTouchEnded(at point: CGPoint, touchType: UITouch.TouchType) {
        ...
        case .canvas(let frame):
            if frame === activeFrame {
                // DRAW ON CANVAS (Active Frame)
                        ...
                        noteCommittedHandwritingStrokeIfNeeded(stroke: stroke, target: routedTarget)
        ...
        case .card(let card, let frame):
            ...
            // Transform points into card-local space accounting for which frame the card is in
                ...
                noteCommittedHandwritingStrokeIfNeeded(stroke: cardStroke, target: target)
        }
    ...
    private func cancelPendingHandwritingRefinement() {
        handwritingRefinementTimer?.invalidate()
        handwritingRefinementTimer = nil
        handwritingRefinementGeneration += 1
    }

    private func resetHandwritingRefinementState() {
        handwritingRefinementTimer?.invalidate()
        handwritingRefinementTimer = nil
        handwritingRefinementPendingTarget = nil
        handwritingRefinementPendingStrokeIDs = []
        handwritingRefinementGeneration += 1
    }

    private func isSameTarget(_ a: DrawingTarget, _ b: DrawingTarget) -> Bool {
        switch (a, b) {
	    ...
	    private func noteCommittedHandwritingStrokeIfNeeded(stroke: Stroke, target: DrawingTarget) {
	        guard handwritingRefinementEnabled else { return }
	        guard brushSettings.toolMode == .paint else { return }

        if let pendingTarget = handwritingRefinementPendingTarget, !isSameTarget(pendingTarget, target) {
            handwritingRefinementPendingStrokeIDs = []
        }
        handwritingRefinementPendingTarget = target
        handwritingRefinementPendingStrokeIDs.append(stroke.id)

	        handwritingRefinementTimer?.invalidate()
	        let generation = handwritingRefinementGeneration
	        NSLog("[HandwritingRefinement] Scheduled (strokes=%d, delay=%.2fs, gen=%d)", handwritingRefinementPendingStrokeIDs.count, handwritingRefinementDelaySeconds, generation)
	        handwritingRefinementTimer = Timer.scheduledTimer(withTimeInterval: handwritingRefinementDelaySeconds, repeats: false) { [weak self] _ in
	            self?.runHandwritingRefinementIfNeeded(generation: generation)
	        }
	    }

    private func runHandwritingRefinementIfNeeded(generation: Int) {
        guard handwritingRefinementEnabled else { return }
        guard generation == handwritingRefinementGeneration else { return }
        guard let target = handwritingRefinementPendingTarget else { return }
        let strokeIDs = handwritingRefinementPendingStrokeIDs
        ...
        handwritingRefinementPendingStrokeIDs = []
	        ...
	        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
	            guard let self else { return }
	            if generation != self.handwritingRefinementGeneration { return }

	            let oldStrokes: [Stroke]
	            switch target {
	            case .canvas(let frame):
	                oldStrokes = strokeIDs.compactMap { id in frame.strokes.first(where: { $0.id == id }) }
		            case .card(let card, _):
		                oldStrokes = strokeIDs.compactMap { id in card.strokes.first(where: { $0.id == id }) }
		            }
		            if oldStrokes.isEmpty { return }

		            // Plan addendum: use OCR to extract the burst's text (for demo-quality beautification).
		            NSLog("[HandwritingRefinement] Running OCR on %d strokes", oldStrokes.count)
		            let ocrRaw = HandwritingOCR.recognizeText(strokes: oldStrokes)
		            let ocrText = ocrRaw.map(HandwritingOCR.normalizeTextForDeepWriting)
		            self.lastHandwritingRefinementOCRText = ocrText
		            if let ocrText {
		                if let ocrRaw, ocrRaw != ocrText {
		                    NSLog("[HandwritingRefinement] OCR text: %@ (normalized: %@)", ocrRaw, ocrText)
		                } else {
		                    NSLog("[HandwritingRefinement] OCR text: %@", ocrText)
		                }
		            } else {
		                NSLog("[HandwritingRefinement] OCR text: <none>")
		            }

			            // Plan addendum (demo-style MVP): OCR → text→stroke beautifier model, then replace strokes fully.
			            if let ocrText, !ocrText.isEmpty {
			                NSLog("[HandwritingBeautify] Running text→stroke model")
			                let modelStrokes = self.handwritingBeautifier.generateStrokes(ocrText: ocrText)
			                if !modelStrokes.isEmpty {
			                    NSLog("[HandwritingBeautify] Model returned %d strokes", modelStrokes.count)
			                    // Compute old bounds in world space.
			                    var oldMin = SIMD2<Float>(Float.greatestFiniteMagnitude, Float.greatestFiniteMagnitude)
			                    var oldMax = SIMD2<Float>(-Float.greatestFiniteMagnitude, -Float.greatestFiniteMagnitude)
			                    for s in oldStrokes {
			                        let ox = Float(s.origin.x)
		                        let oy = Float(s.origin.y)
		                        for p in s.rawPoints {
		                            let wp = SIMD2<Float>(ox + p.x, oy + p.y)
		                            oldMin = SIMD2<Float>(min(oldMin.x, wp.x), min(oldMin.y, wp.y))
		                            oldMax = SIMD2<Float>(max(oldMax.x, wp.x), max(oldMax.y, wp.y))
		                        }
		                    }
		                    let oldSize = oldMax - oldMin
		                    if oldSize.x.isFinite, oldSize.y.isFinite, oldSize.x > 1e-6, oldSize.y > 1e-6 {
		                        // Compute model bounds (in DeepWriting output units).
		                        var genMin = SIMD2<Float>(Float.greatestFiniteMagnitude, Float.greatestFiniteMagnitude)
		                        var genMax = SIMD2<Float>(-Float.greatestFiniteMagnitude, -Float.greatestFiniteMagnitude)
		                        for stroke in modelStrokes {
		                            for p in stroke {
		                                genMin = SIMD2<Float>(min(genMin.x, p.x), min(genMin.y, p.y))
		                                genMax = SIMD2<Float>(max(genMax.x, p.x), max(genMax.y, p.y))
		                            }
		                        }
		                        let genSize = genMax - genMin
			                        if genSize.x.isFinite, genSize.y.isFinite, genSize.x > 1e-6, genSize.y > 1e-6 {
		                            // Scale to fit inside the user's burst bounds (keep aspect ratio).
		                            let scaleY = oldSize.y / genSize.y
		                            let scaleX = oldSize.x / genSize.x
		                            var scale = min(scaleY, scaleX)
		                            if !scale.isFinite || scale <= 0 { scale = 1 }
		                            scale = min(max(scale, 0.01), 2000.0)

		                            let oldCenter = (oldMin + oldMax) * 0.5
		                            let genCenter = (genMin + genMax) * 0.5
		                            let translate = oldCenter - (genCenter * scale)

		                            let worldStrokes: [[SIMD2<Float>]] = modelStrokes.map { stroke in
		                                stroke.map { p in
		                                    SIMD2<Float>(p.x * scale + translate.x, p.y * scale + translate.y)
		                                }
		                            }
		                            if worldStrokes.contains(where: { $0.count >= 2 }) {
		                                NSLog(
		                                    "[HandwritingBeautify] oldSize=(%.3f,%.3f) genSize=(%.3f,%.3f) scaleX=%.2f scaleY=%.2f scale=%.2f",
		                                    oldSize.x,
		                                    oldSize.y,
		                                    genSize.x,
		                                    genSize.y,
		                                ...
		                                DispatchQueue.main.async { [weak self] in
		                                guard let self else { return }
		                                guard self.handwritingRefinementEnabled else { return }
		                                guard generation == self.handwritingRefinementGeneration else { return }

		                                guard let styleStroke = oldStrokes.last else { return }
		                                let radius = Float(styleStroke.worldWidth) * 0.5

		                                var newStrokes: [Stroke] = []
		                                newStrokes.reserveCapacity(worldStrokes.count)

		                                for ptsWorld in worldStrokes {
		                                    guard ptsWorld.count >= 2 else { continue }
		                                    guard let first = ptsWorld.first else { continue }
		                                    let origin = SIMD2<Double>(Double(first.x), Double(first.y))
		                                    let localPoints: [SIMD2<Float>] = ptsWorld.map { p in
		                                        SIMD2<Float>(p.x - first.x, p.y - first.y)
		                                    }

		                                    let segments = Stroke.buildSegments(from: localPoints, color: styleStroke.color)
		                                    let bounds = Stroke.calculateBounds(for: localPoints, radius: radius)
		                                    let stroke = Stroke(
		                                        id: UUID(),
		                                        origin: origin,
		                                        worldWidth: styleStroke.worldWidth,
		                                        color: styleStroke.color,
		                                        zoomEffectiveAtCreation: styleStroke.zoomEffectiveAtCreation,
		                                        segments: segments,
		                                        localBounds: bounds,
		                                        segmentBounds: bounds,
		                                        device: self.device,
		                                        depthID: self.allocateStrokeDepthID(),
		                                        depthWriteEnabled: styleStroke.depthWriteEnabled,
		                                        layerID: styleStroke.layerID,
		                                        sectionID: styleStroke.sectionID,
		                                        maskAppliesToAllLayers: styleStroke.maskAppliesToAllLayers
		                                    )
		                                ...
		                                switch target {
		                                case .canvas(let frame):
		                                    insertionIndex = frame.strokes.firstIndex(where: { oldIDs.contains($0.id) }) ?? frame.strokes.count
		                                    frame.strokes.removeAll { oldIDs.contains($0.id) }
		                                    frame.strokes.insert(contentsOf: newStrokes, at: min(insertionIndex, frame.strokes.count))
		                                case .card(let card, _):
		                                    insertionIndex = card.strokes.firstIndex(where: { oldIDs.contains($0.id) }) ?? card.strokes.count
		                                    card.strokes.removeAll { oldIDs.contains($0.id) }
		                                    card.strokes.insert(contentsOf: newStrokes, at: min(insertionIndex, card.strokes.count))
		                                }

		                                self.pushUndo(.replaceStrokes(oldStrokes: oldStrokes, newStrokes: newStrokes, insertionIndex: insertionIndex, target: target))
		                            }
		                            return
		                            }
		                    }
		            ...
		            // The student model was trained in the IAM coordinate scale (roughly "screen-space deltas").
		            // Strokes in-app are stored in world units (screen deltas / zoom), so scale points back into
		            // model-space before normalizing/inference, then scale refined output back to world units.
		            var modelScaleByID: [UUID: Float] = [:]
	            modelScaleByID.reserveCapacity(oldStrokes.count)
	            let runs: [HandwritingRefiner.StrokeRun] = oldStrokes.map { stroke in
	                let scale = max(stroke.zoomEffectiveAtCreation, 1e-6)
	                modelScaleByID[stroke.id] = scale
	                let scaledPoints = stroke.rawPoints.map { p in
	                    SIMD2<Float>(p.x * scale, p.y * scale)
	                }
	                return HandwritingRefiner.StrokeRun(id: stroke.id, points: scaledPoints)
	            }
	            let refinedByIDScaled = self.handwritingRefiner.refineStrokeRuns(runs)

	            var pairs: [(old: Stroke, new: Stroke)] = []
	            pairs.reserveCapacity(oldStrokes.count)
	            for old in oldStrokes {
	                let rawPoints = old.rawPoints
	                guard let refinedPointsScaled = refinedByIDScaled[old.id] else { continue }
	                let modelScale = modelScaleByID[old.id] ?? 1.0
	                let refinedPoints = refinedPointsScaled.map { p in
	                    SIMD2<Float>(p.x / modelScale, p.y / modelScale)
	                }
	                if refinedPoints.count != rawPoints.count {
	                    continue
	                }
	                let segments = Stroke.buildSegments(from: refinedPoints, color: old.color)
                let bounds = Stroke.calculateBounds(for: refinedPoints, radius: Float(old.worldWidth) * 0.5)
                let newStroke = Stroke(
                    id: old.id,
                    origin: old.origin,
                    worldWidth: old.worldWidth,
                    color: old.color,
                    zoomEffectiveAtCreation: old.zoomEffectiveAtCreation,
                    segments: segments,
                    localBounds: bounds,
                    segmentBounds: bounds,
                    device: self.device,
                    depthID: old.depthID,
                    depthWriteEnabled: old.depthWriteEnabled,
                    layerID: old.layerID,
                    sectionID: old.sectionID,
                    maskAppliesToAllLayers: old.maskAppliesToAllLayers,
                    link: old.link,
                    linkSectionID: old.linkSectionID,
                    linkTargetSectionID: old.linkTargetSectionID,
                    linkTargetCardID: old.linkTargetCardID
                )
                pairs.append((old: old, new: newStroke))
            }
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                var appliedOld: [Stroke] = []
                var appliedNew: [Stroke] = []
                ...
                case .canvas(let frame):
                    for (old, new) in pairs {
                        guard let idx = frame.strokes.firstIndex(where: { $0.id == old.id }) else { continue }
                        frame.strokes[idx] = new
                        appliedOld.append(old)
                        appliedNew.append(new)
                    }
                case .card(let card, _):
                    for (old, new) in pairs {
                        guard let idx = card.strokes.firstIndex(where: { $0.id == old.id }) else { continue }
                ...
                if !appliedOld.isEmpty {
                    self.pushUndo(.refineStrokes(oldStrokes: appliedOld, newStrokes: appliedNew, target: target))
                }
            }
        }
    }
