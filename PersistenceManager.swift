...
final class PersistenceManager {
    ...
    func exportRNNStrokeSequence(rootFrame: Frame,
        ...
        guard !samples.isEmpty else {
            let empty = RNNStrokeSequence(version: 2, points: [])
        ...
        for sample in samples {
            ...
            var prevInStroke: SIMD2<Double>? = nil

            for (idx, p) in pts.enumerated() {
                let base = prevInStroke ?? p
                let dx = safeFloat(p.x - base.x)
                let dy = safeFloat(p.y - base.y)
                let penUp: Float = (idx == 0) ? 1 : 0
                stitched.append([dx, dy, penUp])
                prevInStroke = p
            }
        }

        let dto = RNNStrokeSequence(version: 2, points: stitched)
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys]
