...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "- dx/dy are within-stroke deltas (no cross-stroke 'teleport' jumps)\n"
            "- p=1 on the first point of each stroke (stroke start marker, always [0,0,1])\n"
    ...
    for i, xml_path in enumerate(xml_files, start=1):
        ...
        try:
            ...
            for dx, dy, p in iter_dxdy_p_from_iam_xml(xml_path):
                if p == 1:
                    # Never let stroke-start tokens carry a mean/std offset; keep them as [0,0,1]
                    # so the model never learns "teleport moves".
                ...
                if not math.isfinite(ndx) or not math.isfinite(ndy):
                    continue
                points.append([float(ndx), float(ndy), 0.0])
